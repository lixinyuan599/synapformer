...
from src.test import test_fix, test
...
def main(name):
    ...
    if name=="Hemibrain":
        ...
        path='/data3/lixinyuan/synapse/MICCAI/data/HemiBrain'
...
### SynapseNet_GAT_PointNet_with_attr
    if name=="Hemibrain":
    ...
    for epoch in range(args.epoch):
        ...
        if synapse_judgment==True:
            ...
            for batch_synapse, batch_synpase_index, batch_rand_index,batch_rand_attr,batch_synapse_attr in data_rand(data, device,selected_edge_index):
                chunk=chunk+1
                bad=bad+1
                train_acc, loss=train(model, data, batch_rand_index, batch_rand_attr, batch_synapse, batch_synpase_index, data.train_mask, optimizer, device,scatter_size,batch_synapse_attr) # Manc:23188 Himebrain:21739

                if chunk % 10==0:
                    test_acc=test_fix(model, data, batch_rand_index, batch_rand_attr, batch_synapse, batch_synpase_index, data.test_mask, device,scatter_size,batch_synapse_attr)
                    print(f'epoch:{epoch:03d}, chunk:{chunk:02d},Test_ACC:{test_acc:0.4f}')
                ...
                if loss < best_loss:
                    ...
                    no_improve +=1
            test_acc=test_fix(model, data, batch_rand_index, batch_rand_attr, batch_synapse, batch_synpase_index, data.test_mask, device,scatter_size,batch_synapse_attr)
            print(f'epoch:{epoch:03d} , Test_ACC:{test_acc:0.4f}')
